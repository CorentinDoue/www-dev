(function() {
	...
	var searchElement = document.getElementById('userfield'),
	...
	function getResults(keywords) { // Effectue une requête et récupère les résultats
	    ...
	    xhr.open('GET', './php/autocomplete.php?text='+ encodeURIComponent(keywords));
	...
	function displayResults(response) { // Affiche les résultats d'une requête
        ...
        if (response.length) { // On ne modifie les résultats que si on en a obtenus

        response = response.split('|'); // On parse la réponse de la requête afin d'obtenir les résultats dans un tableau
        var responseLen = Math.min(response.length,7);
	...
	function chooseResult(result) { // Choisi un des résultats d'une requête et gère tout ce qui y est attaché
	    ...
	    result.className = 'result_unfocus'; // On supprime l'effet de focus
	...
	searchElement.addEventListener('keyup', function(e) {
	    ...
	    if (e.keyCode == 38 && selectedResult > -1) { // Si la touche pressée est la flèche "haut"
	        ...
	        divs[selectedResult].className = 'result_unfocus';
